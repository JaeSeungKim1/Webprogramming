...
soup=BeautifulSoup(webpage,'html.parser')   #네이버에 핵심 뉴스 정보 제목이 계속 바뀌는데 주제만 뽑아서 볼 수 있게 하는 방법
#print(soup.find_all('a',"issue"))
for temps in soup.find_all('a',"issue"):
    print('-->',temps.get_text())
print('\n')

print('<<오늘의 음원 종합 TOP 10 >>\n')     #네이버 음원 차트 1등 이름만 추출해내는 것
resp=urllib.request.urlopen('https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=%EC%9D%8C%EC%9B%90%EC%B0%A8%ED%8A%B8')
toptenlist=[]
artistlist=[]
Rank=10
soup = BeautifulSoup(resp,'html.parser')
print(soup.find_all('a',"tit")[0].get_text())
#10개의 순위를 보게 하기 위해 설정해 놓은 것
for topten in soup.find_all('a','tit'):
    toptenlist.append(topten.get_text())
#print(toptenlist)
print(soup.select('.dsc_area a[target="_vibe"]'))
...
def get_company_list(url):
    ...
    resp.encoding=None #이렇게 했는데도 글자가 깨지면 아래 설정 이용해서 하기
